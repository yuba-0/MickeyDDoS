...
if os.name == "nt":
    ...
    if not os.path.exists(winpcap_dir):
        print(
            f'{Fore.MAGENTA}[!] {Fore.YELLOW}Dikkat! "WinPcap" bileşeni yüklü değil!\n    syn, udp ve diğerlerine saldırı yapmak için gereklidir,\n    Yalnızca SMS spamı kullanmak istiyorsanız bunu atlayabilirsiniz\n    Otomatik olarak yüklemek istiyor musunuz? (y/h){Fore.RESET}'
        )
